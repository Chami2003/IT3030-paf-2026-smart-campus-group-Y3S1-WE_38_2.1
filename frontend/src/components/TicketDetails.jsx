...
const TicketDetails = () => {
    ...
    const handleEditComment = async (commentId) => {
        if (editCommentContent.trim()) {
            try {
                const response = await apiClient.put(`/tickets/comments/${commentId}?currentUser=${currentUser.name}`, {
    ...
    const handleDeleteComment = async (commentId) => {
        if (window.confirm("Are you sure you want to delete this comment?")) {
            try {
                await apiClient.delete(`/tickets/comments/${commentId}?currentUser=${currentUser.name}&currentRole=${currentUser.role}`);
    ...
    const handleUpdateTicket = async () => {
        try {
            ...
            const response = await apiClient.put(`/tickets/${id}`, updatedTicket);
            setTicket(response.data);
            fetchHistory();
